...
import { PLAYER_POSITIONS, REMATCH_PARAMS_KEY, REMATCH_LEADER_KEY, DENOMINATION_KEY } from '../utils/constants'
...
const GameView = ({ redirectPath }: GameViewProps) => {
  ...
  const [denomination, setDenomination] = useState(localStorage.getItem(DENOMINATION_KEY) as Denomination || '$')
  ...
  const chooseDenomination = useCallback((denomination: Denomination) => {
    setDenomination(denomination)
    localStorage.setItem(DENOMINATION_KEY, denomination)
  }, [])
            ...
            <Col className="center-table">
              ...
              <Col className='pots'>
                {Boolean(computedPots[0]?.amount) && String(computedPots[0]?.amount) !== '0' && (
                  <Text className='pot'>{game.pots.length > 1 ? 'Main ' : ''}Pot: {dnmAmnt(computedPots[0]?.amount || '0')}</Text>
                )}
              ...
              return (
                <Col className={`player-display ${PLAYER_POSITIONS[`${ind + 1}${playerOrder.length}`]}`} key={p.ship}>
                  ...
                  <div className={cn(
                    ...
                    <Text className='stack' bold>{p.left ? 'Left' : `${denomination}${dnmAmnt(p.stack)}`}</Text>
                  </div>
                  <Row className='bet'>
                    {Boolean(buttonIndicator) && <div className={cn('button-indicator', buttonIndicator === 'D' && 'dealer')}>
                      {buttonIndicator}
                    </div>}
                    {Number(p.committed) > 0 && <div>Bet: {dnmAmnt(p.committed)}</div>}
                  </Row>
                  {curTurn && !isSelf && !hand && (
          ...
          <Col style={{ alignItems: 'flex-end' }}>
            ...
            <Row style={{ marginTop: 4 }}>
              ...
              <Select value={denomination} onChange={e => chooseDenomination(e.target.value as Denomination)}>
                {(['$', 'BB'] as Denomination[]).map(d => <option value={d}>{d}</option>)}
              </Select>
            </Row>
          </Col>
        </Row>
