...
import { createSubscription, handleGameUpdate, handleLobbyUpdate, handleNewMessage, SubscriptionPath } from "./subscriptions/pokurSubs"
...
export interface PokurStore {
  ...
  spectateTable: (table: string, isPublic?: boolean) => Promise<void>
...
const usePokurStore = create<PokurStore>((set, get) => ({
  ...
  createTable: async (values: CreateTableValues) => {
    ...
    if (values['game-type'] === 'cash') {
      json['new-table']['game-type'] = { cash: {
        ...
        'min-buy': values['big-blind']! * values['min-buy'],
        'max-buy': values['big-blind']! * values['max-buy'],
      } }
    } else {
  ...
  spectateTable: async (table: string, isPublic = false) => {
    const json = { 'spectate-table': { id: table, public: isPublic } }
    await api.poke({ app: 'pokur', mark: 'pokur-player-action', json })
  },
