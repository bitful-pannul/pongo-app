...
import { checkIsDm, getMsgText, getChatName, isCallRequest } from '../../../util/ping'
...
export default function ChatsEntry({ chat, navigation }: ChatProps) {
  ...
  const acceptCall = useCallback(() => {
    if (last_message?.author) {
      navigation.navigate('Call', { chatId: id, ship: deSig(last_message.author) })
    }
  }, [navigation, id, last_message])

  const rejectCall = useCallback(() => {
    sendMessage({ self, convo: id, kind: 'webrtc-call', content: 'end' })
  }, [id, self, sendMessage])
  ...
  const messageDisplay = (
    <Col style={{ maxWidth: '100%' }}>
      ...
      (
        <>
          <Row>
            <Text style={{ fontSize: 14 }} numberOfLines={1}>Video Call</Text>
        ...
        <>
          {last_message?.author && !isDm && (
            <Text style={{ fontWeight: '600' }}>
              {deSig(last_message?.author || '') === deSig(self) ? 'You' : chat?.last_message?.author}
            </Text>
          )}
