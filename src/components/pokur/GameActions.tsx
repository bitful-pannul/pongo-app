...
const GameActions = ({ pots, secondsLeft, denomination, bigBlind }: GameActionsProps) => {
  ...
  const { currentBet, committed, isTurn, canCheck, myTotal, maxBet, minBet, minRaise, callAmountDisplay, callAmountChips } =
  ...
  const betAction = useCallback(async () => {
    if (game && myBet && myBet > 0) {
      if ((myBet - committed) < minBet && myBet !== maxBet) {
        return window.alert(`Your raise must be at least ${minBet + committed}`)
      }
      await lockActions(bet(game.id, getAmount(Math.min(maxBet, myBet) - committed)))
      setMyBet(minRaise)
    }
  }, [game, myBet, maxBet, committed, minRaise, minBet, bet, lockActions, getAmount])
